...
class GrantedResourceFlowTestCase(BaseFlowTestCase):
    def test_granted_app_and_resource_query_flow(self) -> None:
        ...
        try:
            ...
            missing_tree_param_body = ensure_api_error(missing_tree_param_response, 400)
            self.assertTrue(
                "租户id不能为空" in missing_tree_param_body["msg"]
                or "参数无法绑定，请检查格式和字段" in missing_tree_param_body["msg"],
                f"unexpected grant/app/tree validation msg: {missing_tree_param_body['msg']}",
            )
            ...
            before_granted_app_body = ensure_api_error(before_granted_app_response, 403, 11)
            self.assertIn("权限不足", before_granted_app_body["msg"])
            ...
            missing_app_id_body = ensure_api_error(missing_app_id_response, 400)
            self.assertTrue(
                "应用id不能为空" in missing_app_id_body["msg"]
                or "参数无法绑定，请检查格式和字段" in missing_app_id_body["msg"],
                f"unexpected granted/app/res-tree validation msg: {missing_app_id_body['msg']}",
            )
            ...
            granted_app_after_revoke_body = ensure_api_error(granted_app_after_revoke_response, 403, 11)
            self.assertIn("权限不足", granted_app_after_revoke_body["msg"])
        finally:
